...
| `openai_adapter_http_test.rs`                            | Actual OpenAI adapter Chat Completions wire, provider-reported model, selected-target cost, missing-price failure, scripted 500, inherited env/proxy isolation                                        |
| `openai_results_http_test.rs`                            | Production-router Chat Completions protocol plus two named targets that share a reported model but keep their own configured prices                                                                   |
