...
| `admission_limits_http_test.rs`                          | Protected-router raw-body and metadata bounds: inclusive `max_request_body_bytes` on declared `Content-Length` and chunked/no-length bodies for inference and key-create JSON, advertised oversize `413 PAYLOAD_TOO_LARGE` envelopes with correlation, metadata `max_metadata_bytes` `400 INVALID_REQUEST`, zero generation calls on rejection, and `/health`/`/ready` remaining unrestricted                                                                                                                                                                                                                                                                                                              |
