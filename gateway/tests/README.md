...
| `openai_adapter_http_test.rs`                            | Actual OpenAI adapter Chat Completions wire, provider-reported model, selected-target cost, missing-price failure, malformed/invalid success payloads, bounded and chunked bodies, scripted 500, inherited env/proxy isolation |
| `openai_results_http_test.rs`                            | Production-router Chat Completions protocol, named-target prices, malformed success payloads without retry, and response-size bounds across Content-Length and chunked transfer                                                |
